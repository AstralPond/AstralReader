...
export default function useAuth(s?: Store<"main", State>, r?: Router) {
  ...
  return {
    auth,
    login: login(store, router),
    signup: signup(store),
    fetchUserData: () => loadCookie(store),
  };
}
...
function login(store: Store, router: Router) {
  return async (body: UserInfoBody) => {
    try {
...
function signup(store: Store) {
  return async (body: UserInfoBody) => {
    try {
      const response = await fetch("http://localhost:3000/gatekeeper/signup", {
        method: "POST",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
      ...
      if (response.status === 200) {
        store.$patch((state: State) => {
          state.auth.user = { email: body.email };
        });
      }
    } catch (e) {
      console.log(e);
